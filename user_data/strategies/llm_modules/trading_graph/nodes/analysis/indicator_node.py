"""
Indicator Analysis Node for LangGraph.

Technical indicator analysis: RSI, MACD, ADX, Stochastic, MFI.
Runs in parallel with other analysis nodes in the analysis subgraph.
"""
...
from ....prompts.analysis.indicator_prompt import (
...
def indicator_node(state: AnalysisState, config: RunnableConfig | None = None) -> Dict[str, Any]:
    """
    Technical indicator analysis node.

    Analyzes RSI, MACD, ADX, Stochastic, MFI indicators and generates
    trading signals with direction and confidence.
    ...
    Returns:
        Dict with agent_reports and indicator_report updates
    """
    pair = state.get("pair", "UNKNOWN")
    market_context = state.get("market_context", "")

    logger.debug(f"[IndicatorAgent] Starting analysis for {pair}")

    try:
        # Get LLM config from state first, then fallback to graph config
        llm_config = state.get("llm_config") or {}
        if not llm_config and config:
            llm_config = config.get("configurable", {}).get("llm_config", {})

        # Create LLM instance
        llm = LLMFactory.create_chat_model(llm_config, task_type="indicator")

        # Build prompt
        prompt = build_indicator_prompt(market_context)
        ...
        # Parse response
        parsed = _parse_indicator_response(response_text)

        # Create report
        report = AgentReport(
            agent_name="IndicatorAgent",
        ...
        logger.info(
            f"[IndicatorAgent] {pair} analysis complete: "
        ...
        logger.error(f"[IndicatorAgent] Analysis failed: {e}")
...
def _create_error_result(error_msg: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "indicator_report": error_report,
        "errors": [f"IndicatorAgent: {error_msg}"]
    }


def _parse_indicator_response(response: str) -> Dict[str, Any]:
    """
    Parse LLM response to extract signals, direction, confidence.

    Args:
        response: LLM response text

    Returns:
        Parsed dict with signals, direction, confidence, key_levels
    """
    result = {
        "signals": [],

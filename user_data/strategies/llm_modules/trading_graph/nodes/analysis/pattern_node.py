"""
Pattern Recognition Node for LangGraph.

K-line pattern recognition: candlestick patterns, chart patterns.
...
from ....prompts.analysis.pattern_prompt import (
    PATTERN_SYSTEM_PROMPT,
    build_pattern_prompt,
    build_pattern_vision_prompt,
    build_pattern_multi_tf_vision_prompt,
)
...
def pattern_node(state: AnalysisState, config: RunnableConfig | None = None) -> Dict[str, Any]:
    """
    K-line pattern recognition node.
    ...
    Returns:
        Dict with agent_reports and pattern_report updates
    """
    ...
    logger.debug(f"[PatternAgent] Starting analysis for {pair}")
    ...
    try:
            ...
            try:
                ...
                if ohlcv_data_htf is not None and multi_tf_enabled:
                    ...
                    if chart_result and chart_result.get("success") and chart_result.get("image_base64"):
                        ...
                        logger.debug(f"[PatternAgent] Single-TF Vision mode enabled")

            except Exception as e:
                logger.debug(f"[PatternAgent] Chart generation failed: {e}, using text mode")
                use_vision = False
                chart_images = []
        ...
        if use_vision and chart_images:
            try:
                ...
                if use_multi_tf and len(chart_images) >= 2:
                    prompt = build_pattern_multi_tf_vision_prompt(
                    ...
                    prompt = build_pattern_vision_prompt(market_context, pair)
                ...
                logger.warning(f"[PatternAgent] Vision call failed: {e}, falling back to text mode")
                use_vision = False
                use_multi_tf = False
                response = None
        ...
        if response is None:
            llm = LLMFactory.create_chat_model(llm_config, task_type="pattern")
            prompt = build_pattern_prompt(market_context)
        ...
        # Parse response
        parsed = _parse_pattern_response(response_text)
        ...
        logger.info(
            f"[PatternAgent] {pair} {mode_tag} analysis complete: "
        ...
        logger.error(f"[PatternAgent] Analysis failed: {e}")
...
def _create_error_result(error_msg: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "pattern_report": error_report,
        "errors": [f"PatternAgent: {error_msg}"]
    }


def _parse_pattern_response(response: str) -> Dict[str, Any]:

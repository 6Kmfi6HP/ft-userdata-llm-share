...
class StoplossCalculator:
    ...
    def calculate_stoploss_price(
        ...
        """
        # 默认配置
        # 🔧 修复逻辑冲突：根据 Freqtrade 官方最佳实践，高盈利区间应放宽止损以"让利润奔跑"
        # 盈利>15%时：从0.8×ATR放宽到2.0×ATR，最小距离从0.5%放宽到3.0%
        # 盈利6-15%时：保持1.0×ATR，最小距离从1.0%放宽到2.0%
        # 盈利2-6%时：保持1.5×ATR，最小距离保持1.5%
        default_config = {
            'profit_thresholds': [0.02, 0.06, 0.15],
            'atr_multipliers': [1.5, 1.0, 2.0],  # 盈利>15%时放宽到2.0×ATR（原0.8）
            'min_distances': [0.015, 0.020, 0.030],  # 盈利>15%时最小3.0%（原0.5%），6-15%时2.0%（原1.0%）
            'time_decay_hours': 2,
            'time_decay_factor': 0.8,
            'trend_strength_threshold': 25,
            'trend_strength_factor': 1.2,
            'use_smooth_transition': True,
            'max_atr_pct': 0.10  # ATR上限10%
        }

...
分析要点：
1. 趋势方向：多时间框架的趋势是否一致
2. 价格位置：当前价格相对于支撑/阻力位的位置
3. 结构完整性：关键位是否被突破
4. 风险收益比：到止损位的距离 vs 到目标位的距离
5. 持仓状态：如有持仓，当前浮盈/浮亏情况
...
系统综合恐惧贪婪指数和资金费率的分类：
- bullish：多项指标显示极端看跌情绪
- bearish：多项指标显示极端看涨情绪
- neutral：情绪中性
...
- 关键阻力位: 105000
- 安全边距: 1.5 × 300 = 450
- 止损触发价: 105000 + 450 = 105450 [正确]

示例（做多，ATR=15）：
- 关键支撑位: 200
- 安全边距: 1.5 × 15 = 22.5
- 止损触发价: 200 - 22.5 = 177.5 [正确]
...
重要概念澄清：
【价格容错空间 ≠ 到关键位的距离】
- 错误理解：当前价格到支撑位/阻力位的距离 = 止损空间 [错误]
- 正确理解：止损空间 = 从入场价到止损触发价的距离 [正确]

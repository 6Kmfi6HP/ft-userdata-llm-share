...
- patterns 列表中每个形态必须有 name 和 confidence（0.0-1.0）
- 如果没有明显形态，patterns 可以为空数组"""
...
**图表说明**：
- 上半部分：带趋势线的价格图（蓝色=支撑线，红色=阻力线）
- 下半部分：标准 K 线图（用于形态识别）

**请完成以下综合分析**：

1. **趋势线分析**（基于上半部分）：
   - 价格与趋势线的互动关系（反弹、突破、假突破）
   - 趋势线斜率和间距（收敛/发散）
   - 支撑/阻力线的有效性

2. **K线形态识别**（基于下半部分）：
   - 识别经典形态（如果有明显形态）
   - 给出形态名称和置信度评分（0.0-1.0）
   - 说明形态的完成度和有效性
   - 如果没有明显形态，可以 patterns 为空数组

3. **趋势判断**（综合两部分）：
   - 判断短期趋势方向：up（上涨）/ down（下跌）/ sideways（横盘）
   - 给出置信度评分（0.0-1.0）
   - 列举 2-3 条支撑证据（价格行为、趋势线互动、形态特征等）

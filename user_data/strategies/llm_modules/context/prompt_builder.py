...
class PromptBuilder:
    ...
    def build_entry_prompt(self) -> str:
        ...
        parts = [
            ...
            "  2. 入场计划：若开仓，说明方向、触发条件、目标/失效位、杠杆与 stake_amount（根据上述资金管理原则计算）。",
...
多空比的正确理解：
  ...
  恐惧指数低 + 多空比高：散户恐慌但不愿割肉，被套牢状态，不是矛盾。
  贪婪指数高 + 多空比高：散户追涨买入，可能接近顶部。
...
部分止盈的实现方式 (使用 adjust_position 工具):
  • 使用 adjust_position 的负数参数进行减仓止盈
  • 例如: adjustment_pct=-30 表示减仓30%,锁定30%利润,保留70%追趋势
  • 例如: adjustment_pct=-50 表示减仓50%,平衡利润保护与趋势参与
...
注意: 配置中的 minimal_roi 仅作为极端情况的保底:
  • 50% 极限止盈 (防止暴涨不离场)
  • 24小时 15% 强制止盈 (避免长期僵持)
  • 72小时 5% 最低止盈 (必须行动)
  
绝大多数情况下，应该由你根据市场状态主动决策止盈时机。"""

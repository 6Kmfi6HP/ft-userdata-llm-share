...
class PromptBuilder:
    ...
    def build_entry_prompt(self) -> str:
        ...
        parts = [
            ...
            "【风险收益比要求】",
            "  - 任何开仓信号须先验证 R:R ≥ 1.8",
            "  - R(盈利潜力) = (目标价 - 入场价) / 入场价 × 杠杆",
            "  - R(风险) = (入场价 - 止损价) / 入场价 × 杠杆",
            "  - R:R = R(盈利潜力) / R(风险)",
            "  - 若 R:R < 1.8 → 放弃该信号，调用 signal_wait",
            "  - 说明计算过程：列出目标价、止损价、杠杆、R:R值",
            "",
            "【输出要求】",
            "  1. 市场观察：引用关键数据总结方向/结构/波动，不延伸个人偏好。",
            "  2. 入场计划：若开仓，说明方向、触发条件、目标/失效位、杠杆与 stake_amount（结合账户余额与置信度）。",
            "  3. 风险收益验证：明确计算并展示 R:R 值，确保 ≥ 1.8。",
            "  4. 决策：只能调用 signal_entry_long / signal_entry_short / signal_wait，并写明理由。",
            "",
            "分析完毕后立即调用一个函数完成决策。",
        ]
    ...
    def build_position_prompt(self) -> str:
        ...
        parts = [
            ...
            "",
            "目标：趋势优先，但盈利不可出现过度回撤。",
            "开仓理由是你的锚点，趋势延续时继续持有，但必须主动保护已获利润。",
            "保护动作优先（移动保护位/分批止盈），在锚点未破但回撤过度时先减仓再观测。",
            ...
            "【记住】",
            "",
            "你是趋势交易员，追求完整趋势，但不允许利润大幅回吐。",
            "给趋势时间成长，但当dd_ratio触发保护阈值时，立即执行分批减仓。",
            "利润保护不等于离场：可用adjust_position减仓25%-50%，同时移动保护位，继续观测剩余仓位。",
            "全部退出仅在：保护位触发 或 结构+动量反转确认 或 hold_pattern='stuck_in_loop'且dd_ratio≥0.5。",
...
盈利回撤参考（需根据数据可用性判断）：
  【若上下文显示"✅持仓追踪数据可用"】：
    查看'最大浮盈(MFE)'和'当前盈亏(CP)'。
    如果盈利从高点大幅回撤（如MFE=+25%降至CP=+6%，回撤76%），说明市场可能转向。
    可以考虑减仓保护部分利润，或在锚点破坏时平仓。
  
  【若上下文显示"⚠️【持仓追踪数据不可用】"】：
    改用持仓时间和当前盈亏百分比判断：
    持仓>6小时且盈利<3% → 考虑离场（效率低下）
    盈利曾达到较高水平但现在回落 → 结合价格位置和指标判断是否减仓
    优先基于锚点是否破坏和趋势是否反转来决策"""
...
需要警惕：
  ...
  • 价格突破摆动高/低点时，幅度≥0.5%且连续≥2根收盘价站稳才可认定结构变化；<0.3%的突破视为噪声回调。
  • 单根影线或瞬时指标跳变不得作为趋势反转依据，必须配合关键位/均线突破或动量翻转。
  • 做空持仓未收盘站上锚点均线（如EMA20/关键阻力），或做多持仓未跌破支撑，就不能以"Higher Low/Lower High"之类孤立信号平仓。
  • 任何平仓/反转结论至少需要"结构 + 关键位/EMA + 动量"三者中的两项共同确认。"""

    def _profit_protection_rules(self) -> str:
        """动态利润保护规则（分级止盈+dd_ratio触发）"""
        return """【动态利润保护（务必执行）】

总则:
  - 趋势优先,但盈利不可出现过度回撤
  - 优先使用"减仓"锁定阶段利润（系统有15%硬性止损+动态止损作为最后防线）
  - 仅在结构/动量确认反转时全部退出

核心变量（通常由上下文提供，需检查可用性）:
  【若上下文显示"✅持仓追踪数据可用"】:
    - MFE = 最大浮盈%; CP = 当前盈亏%; DD = MFE - CP
    - dd_ratio = DD / max(MFE, 1e-6)  ← 动态保护的核心指标
    - 严格按dd_ratio阈值执行分级减仓（见下方分级动作）
    - ATR% 通常可用,若显示"数据不可用"则ATR倍数参考失效
  
  【若上下文显示"⚠️【持仓追踪数据不可用】"】:
    - 无法使用MFE/dd_ratio保护规则
    - 改用当前盈亏百分比判断回撤 (如CP从+10%跌至+3% = 70%回撤)
    - 建议更保守的持仓管理,降低风险容忍度
    - 仅基于价格、指标、持仓时长做决策
  
  【key_swing和ATR】:
    - key_swing = 摆动高点/摆动低点（见【市场结构与关键指标】,通常总是提供）
    - ATR% 若显示"数据不可用",则建议止损位中的ATR倍数参考无效,仅用swing

建议止损位参考（非强制，系统有动态止损保护）:
  - 做多持仓: MFE≥5% → 关注盈亏平衡+0.2×ATR%; MFE≥10% → 关注swing_low - 0.6×ATR%作为风险线
  - 做空持仓: MFE≥5% → 关注盈亏平衡+0.2×ATR%; MFE≥10% → 关注swing_high + 0.6×ATR%作为风险线
  - 若ATR%不可用: 直接使用swing_low/swing_high作为关键风险线

分级减仓动作(使用adjust_position函数) - 仅在有MFE/dd_ratio时生效:
  级别A(5%≤MFE<10%):
    触发: dd_ratio>0.35 或 价格跌破EMA20且ADX下降≥3根
    动作: adjust_position -15%~-25%, 在分析中说明建议的风险防线

  级别B(10%≤MFE<20%):
    触发: dd_ratio>0.30
    动作: adjust_position -25%~-35%, 说明基于key_swing的风险评估

  级别C(MFE≥20%):
    触发: dd_ratio>0.30 或 MACD柱状图连续3根走弱
    动作: adjust_position -35%~-50%, 说明对剩余仓位的风险容忍度

  【若无dd_ratio数据的降级策略】:
    - 盈利回撤>50% (如+10%→+5%) → 减仓20%-30%
    - 盈利回撤>70% (如+10%→+3%) → 减仓40%-50%
    - 持仓时间>6小时且盈利<3% → 考虑离场

退出条件(不等锚点完全破坏):
  - 结构+动量反转: 击穿key_swing + MACD反向扩张
  - hold_pattern='stuck_in_loop'且dd_ratio≥0.5 → 至少减仓50% (若无dd_ratio则看hold次数>5)
  - 系统动态止损会在盈利回撤严重时自动触发

输出要求:
  - 若有【持仓追踪数据】: 明确给出MFE, CP, dd_ratio,说明减仓触发条件
  - 若【持仓追踪数据不可用】: 说明基于当前盈亏和时长判断,使用降级策略
  - 说明: 当前距离swing_high/swing_low的距离, 减仓比例及理由
  - 若选择hold,说明"可接受的最大回撤"及"下次减仓触发条件"
"""
